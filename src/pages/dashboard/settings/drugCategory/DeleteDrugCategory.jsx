...
const DeleteDrugCategory = ({ closeDeleteModal, id }) => {

    const handleDelete = async (id) => {
        try {
            console.log(id);      
            const res = await axios.delete(`${baseUrl}/drug_category.php?id=${id}`);
            if (res.data.status === "success") {
                ...
                Alert(res.data.status, res.data.message)
                console.error('Failed to delete category:', res.data);
            }
        } catch (error) {
            console.error('An error occurred while deleting category:', error.message);
        } finally {
            closeDeleteModal();
        }
    };
            ...
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                    ...
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 className="text-lg mt-2 leading-6 font-medium text-gray-900" id="modal-title">Delete Category</h3>
...
export default DeleteDrugCategory;
